...
const Cart = () => {
  const { cartItem, incrementQuantity, decrementQuantity,removeItem, total} = cart();
  return (
      ...
      {cartItem.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3  py-10 container mx-auto">
              ...
              >
                ...
                <div className="flex flex-col items-center gap-10">
                  <div className="flex items-center gap-10">
                    <p> <span className="font-semibold">Total:</span> ${itemCart.price * itemCart.quantity} </p>
                    <button className="text-red-500 text-2xl">
                      <MdRemoveShoppingCart  onClick={() => removeItem(itemCart)}/>
                    </button>
                  </div>
                  <div className="flex items-center gap-7">
                    <button onClick={() => decrementQuantity(itemCart)} className=" shadow text-[17px] px-2 rounded-md">
                      -
                    </button>
                    <span className="mb-2">{itemCart?.quantity}</span>
                    <button onClick={ () => incrementQuantity(itemCart)} className=" shadow text-[17px] px-2 rounded-md">
                      +
                    </button>
                  </div>
                </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md h-fit flex flex-col gap-7">
            ...
            <button className="bg-[#FB923C] text-white py-2 px-4 rounded-md mt-4 w-full">
              Checkout
            </button>
          </div>
        </div>
